...
$pythonValidatorPath = Join-Path $PSScriptRoot "validate_platform_topology.py"
...
if (-not (Test-Path -LiteralPath $pythonValidatorPath -PathType Leaf)) {
    throw "Validateur Python absent: scripts/validate_platform_topology.py"
}
...
if (-not $PSBoundParameters.ContainsKey("Path")) {
    $Path = Join-Path $repoRoot "app/platform/topology_registry.json"
}
