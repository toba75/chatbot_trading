...
elseif ($env:OST_M013_PRECONDITION_ACCEPTANCE_RUNNING -eq "1") {
    $expectedTestCount = 269
}
$expectedTestSummary = "Gate test GREEN: 29 validation(s), $expectedTestCount test(s)."
...
try {
    ...
    Assert-OutputContains -Output $lintResult.Output -Expected "Gate lint GREEN: 30 validation(s), 0 test(s)." -Message "La gate de lint doit prouver le nombre exact de validations et tests."
