...
Assert-MatchInReport `
    -Content $content `
    -Pattern "Given le dépôt ``master`` contient la spécification v4\.1 et le registre ADR\." `
    -Message "Le scénario BDD ne contient pas le Given attendu."

Assert-MatchInReport `
    -Content $content `
    -Pattern "When la précondition de M-000 est vérifiée\." `
    -Message "Le scénario BDD ne contient pas le When attendu."

Assert-MatchInReport `
    -Content $content `
    -Pattern "Then l'état des validations existantes, des commandes absentes et des tâches versionnées est déclaré sans ambiguïté\." `
    -Message "Le scénario BDD ne contient pas le Then attendu."

Assert-MatchInReport `
    -Content $content `
    -Pattern "\*\*Révision master observée :\*\* ``[0-9a-f]{40}``" `
