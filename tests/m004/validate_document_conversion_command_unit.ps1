...
class ScriptedDocumentConversionCommands:
    def __init__(self):
...
conversion_signature = inspect.signature(DocumentConversionCommandService.request_document_conversion)
if conversion_signature.parameters["document_id"].default is not inspect.Parameter.empty:
    raise AssertionError("request_document_conversion.document_id ne doit pas avoir de valeur par d̩faut.")
init_signature = inspect.signature(DocumentConversionCommandService.__init__)
if init_signature.parameters["conversion_configuration_hash"].default is not inspect.Parameter.empty:
    raise AssertionError("conversion_configuration_hash ne doit pas avoir de valeur par d̩faut.")
...
if JOB_RUNTIME_CATALOG.includes("CanonicalSourcePublished"):
    raise AssertionError("Un event type ne doit pas ̻tre accept̩ comme job.")
...
assert_equal(
    ...
    "CVER-M004-T009-0001",
    "canonical_version_id doit ̻tre autoris̩ seulement apr̬s acceptation canonique.",
)
assert_raises(
    ValueError,
...
# La commande applicative cr̩e un job CONVERT_DOCUMENT idempotent et ne retourne pas l'identit̩ technique du job.
conversion = service.request_document_conversion(document_id=document.document_id.value)
assert_equal(conversion.document_id, document.document_id, "La conversion doit conserver le DocumentId m̩tier.")
assert_equal(conversion.conversion_status, DocumentConversionStatus.CONVERSION_REQUESTED, "Le statut public doit ̻tre explicite.")
assert_equal(conversion.canonical_version_id, None, "La version canonique ne doit pas ̻tre expos̩e avant acceptation.")
assert_true(not hasattr(conversion, "job_id"), "La r̩ponse applicative ne doit pas exposer le job_id.")
assert_true(not hasattr(conversion, "processing_run_id"), "La r̩ponse applicative ne doit pas exposer le processing_run_id.")
...
assert_equal(pending_job.request.job_name, "CONVERT_DOCUMENT", "Le job de conversion doit ̻tre global et explicite.")
assert_equal(pending_job.request.idempotence_key.job_name, "CONVERT_DOCUMENT", "La cl̩ d'idempotence doit nommer le job de conversion.")
...
assert_equal(len(conversion_repository.submitted_conversion_requests), 1, "La demande doit ̻tre persist̩e une seule fois.")
...
assert_equal(job_queue.created_job_count(), 1, "La r̩p̩tition ne doit pas cr̩er un second job.")
...
conversion_repository.conversions_by_document_id[qa_rejected_document.document_id.value] = DocumentConversionState(
...
# L'adaptateur HTTP ne d̩cide ni route ni autorit̩; il transmet seulement le DocumentId.
scripted_commands = ScriptedDocumentConversionCommands()
adapter = SourceProcessingConversionHttpAdapter(document_conversion_commands=scripted_commands)
...
assert_equal(
    ...
    "Le corps HTTP doit rester minimal tant que la version canonique n'est pas accept̩e.",
)
...
assert_equal(
    ...
    "canonical_version_id doit apparątre seulement apr̬s acceptation canonique.",
)
...
assert_equal(not_routed_response.body, {"error_code": "SOURCE_NOT_ROUTED", "document_id": "DOC-2222222222222222"}, "Le code route absente doit ̻tre stable et ne pas exposer le statut interne.")
...
assert_equal(invalid_body.status_code, 400, "Le body de conversion doit ̻tre refus̩.")
assert_equal(
    len(scripted_commands.conversion_calls),
    conversion_call_count_before_invalid_body,
    "Le body invalide ne doit pas d̩clencher une nouvelle commande.",
)
...
# Le domaine SP ne d̩pend d'aucun framework HTTP.
