...
try {
    ...
    $missingBehaviorSpecPath = New-TemporarySpec `
        ...
        -Content ($validContent.Replace("KA-007 - Recherche hybride traçable", "KA-007 - Recherche hybride incomplète"))
    ...
    Assert-OutputContains -Output $missingBehaviorResult.Output -Expected "Comportement attendu absent: KA-007 - Recherche hybride traçable" -Message "Le comportement absent doit être nommé."
    ...
    Assert-OutputContains -Output $missingEventsResult.Output -Expected "Événement attendu absent: KnowledgeProjectionBecameSearchable" -Message "L'événement absent doit être nommé."
    ...
    $qdrantAsSourceSpecPath = New-TemporarySpec `
        -Name "qdrant-source" `
        -Content ($validContent + "`nQdrant est la source de vérité documentaire de KA.`n")
    ...
    Assert-OutputContains -Output $qdrantAsSourceResult.Output -Expected "Qdrant source de vérité interdit" -Message "La confusion Qdrant doit être nommée."
    ...
    Assert-OutputContains -Output $directRaQdrantResult.Output -Expected "Accès RA direct à Qdrant interdit" -Message "L'accès direct doit être nommé."

    $scoreTruthSpecPath = New-TemporarySpec `
        -Name "score-truth" `
        -Content ($validContent + "`nLe score hybride est une vérité métier suffisante pour décider une affirmation.`n")
    ...
    Assert-OutputContains -Output $scoreTruthResult.Output -Expected "Score traité comme vérité interdit" -Message "Le score-vérité doit être nommé."
}
finally {
