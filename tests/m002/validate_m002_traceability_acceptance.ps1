...
$journalPath = Join-Path $repoRoot "docs/tasks/milestone_002/journal.md"
$temporaryRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("ost_m002_traceability_acceptance_" + [System.Guid]::NewGuid().ToString("N"))
$eAcute = [char] 0x00E9
...
if (-not (Test-Path -LiteralPath $journalPath -PathType Leaf)) {
    throw "Journal M-002 absent: docs/tasks/milestone_002/journal.md"
}
...
try {
    ...
    Assert-ExitCode -Actual $validResult.ExitCode -Expected 0 -Message "La matrice M-002 complète doit être acceptée."
    ...
    Assert-ExitCode -Actual $coveredWithoutCommandResult.ExitCode -Expected 1 -Message "Une ligne M-002 couverte sans commande doit être refusée."
    Assert-OutputContains `
        -Output $coveredWithoutCommandResult.Output `
        -Expected "REQ-M002-011" `
...
Write-Host "Test d'acceptation de la tra$($cCedilla)abilit$($eAcute) M-002: OK"
