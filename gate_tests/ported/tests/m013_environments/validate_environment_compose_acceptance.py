...
def test_environment_compose_acceptance() -> None:
    ...
    for environment in ENVIRONMENTS:
        ...
        expected_application_secrets = {
            ...
            "llm-gateway": set(),
            "granite-docling": set(),
            "embedding-service": set(),
            "reranker-service": set(),
            "worker-documents": {"postgres_password"},
            "worker-projection": {"postgres_password", "qdrant_api_key"},
        }
        assert set(expected_application_secrets) == set(APPLICATION_SERVICE_IDS)
        for service_id in APPLICATION_SERVICE_IDS:
            ...
            assert set(mounted_secrets) == expected_application_secrets[service_id]
            assert mounted_secrets == {
                secret_id: f"/workspace/config/secrets/{environment}/{secret_id}"
                for secret_id in expected_application_secrets[service_id]
            }
        ...
        assert set(document.get("secrets", {})) == {
            "local_api_token",
            "postgres_password",
            "qdrant_api_key",
        }
        ...
        assert worker_mounts["/var/run/ocr-docker"]["type"] == "volume"
        assert worker_mounts["/var/run/ocr-docker"]["source"] == "ocr-runtime-socket"
        ocr_mounts = {mount["target"]: mount for mount in ocr_runtime["volumes"]}
        assert ocr_mounts["/var/run/ocr-docker"]["source"] == "ocr-runtime-socket"
