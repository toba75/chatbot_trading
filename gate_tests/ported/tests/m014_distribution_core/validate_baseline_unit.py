...
def _valid_payload() -> dict[str, object]:
    return {
        ...
        "git_references": {
            "master": HASH_A,
            "baseline_branch": HASH_B,
            "granite_cuda": HASH_C,
            "worker_memory_limit": HASH_D,
        },
        "environment": {
            "profile": "test",
        ...
        "inventory": {
            ...
            "tables": ["platform_jobs", "source_processing_runs"],
            "migrations": [
                "app/platform/migrations/008_claim_fencing_and_projection_replay.sql"
            ],
            "configurations": [
        ...
        "network_scope": {
            ...
            "excluded_capabilities": ["ssh", "kamal", "colima", "arm64", "remote_worker"],
        },
        "historical_context": {
...
def test_baseline_complete_est_acceptee() -> None:
    validate_distribution_baseline(_valid_payload())


def test_commit_absent_est_refuse() -> None:
...
def test_metrique_absente_est_refusee() -> None:
    payload = _valid_payload()
...
def test_duree_non_positive_est_refusee() -> None:
    payload = _valid_payload()
...
def test_hash_invalide_est_refuse() -> None:
    payload = _valid_payload()
...
def test_sorties_divergentes_sont_refusees() -> None:
    payload = _valid_payload()
...
def test_limite_memoire_differente_de_deux_gio_est_refusee() -> None:
    payload = _valid_payload()
...
def test_absence_de_preuve_cuda_est_refusee() -> None:
    payload = _valid_payload()
...
@pytest.mark.parametrize("forbidden", ["ssh", "kamal", "colima", "arm64", "remote_worker"])
def test_capacite_distante_active_est_refusee(forbidden: str) -> None:
    payload = _valid_payload()
...
def test_preuve_synthetique_est_refusee() -> None:
