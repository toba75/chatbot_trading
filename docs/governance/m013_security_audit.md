...
- ADR applicables: ADR-007, ADR-008, ADR-009, ADR-014
- ADR: ADR-014 remplace le transport Spark avec clé API et CA obligatoires par un endpoint Docker Spark externe sans clé API, tout en conservant la topologie locale à deux plans, le chemin LLM par gateway et le Spark sans état métier.
...
| `deploy/spark-firewall/network-boundary.json` | Allow-list Spark, auth none, TLS disabled explicite, refus navigateur, refus workers et callbacks | Conforme: une seule règle autorise `llm-gateway -> spark-inference`. |
| `scripts/validate_network_boundary.ps1` | Validation M-002 réutilisée comme socle | Conforme: ports, transport Spark et egress sont contrôlés avant les contrôles M-013. |
...
| CTRL-M013-NET-005 | Seul chemin Spark autorisé | La règle unique est `llm-gateway -> spark-inference` vers `gemma-vllm:8000`. |
...
| CTRL-M013-NET-008 | TLS Spark disabled explicite | `GEMMA_TLS_MODE` vaut `disabled`, aucun `GEMMA_CA_BUNDLE` n'est monté et la politique pare-feu déclare `tls_required=false`. |
| CTRL-M013-NET-009 | Authentification Spark none explicite | `GEMMA_AUTH_MODE` vaut `none`, aucun `GEMMA_API_KEY_FILE` n'est monté et aucun header `Authorization` n'est requis par le conteneur Gemma. |
...
|---|---|---|
| `llm-gateway -> spark-inference` | autorisé | Chemin applicatif unique vers vLLM selon ADR-014. |
...
- Aucun secret Spark fictif n'est requis pour l'inférence actuelle; `llm-gateway` reçoit seulement l'endpoint explicite, le modèle servi et les modes `none` / `disabled`.
