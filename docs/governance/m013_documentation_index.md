...
| docs/runbooks/sauvegarde_restauration.md | Vérification d'archive chiffrée et restauration isolée selon `M013-BackupManifest-1.1`. | `uv run --locked backup-v1`; `uv run --locked restore-v1`; `uv run --locked pytest gate_tests/ported/tests/m013_environments/validate_backup_restore_runtime_unit.py`; `uv run --locked pytest gate_tests/ported/tests/m013_environments/validate_backup_restore_compose_live.py` | `docs/governance/m013_environments_backup_restore_smoke.md`; `docs/adr/ADR-047-archive-chiffree-verifiee-avant-preuve-restauration.md` |
