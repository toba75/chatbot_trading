...
- Tests d'acceptation à écrire: `uv run --locked gate`, couvrant exigences, tests, scripts, docs, ADR-016, rapport d'audit et absence de clôture implicite de M-013.
- Tests unitaires à écrire: `uv run --locked gate`, couvrant mapping exigence-test-code-doc, comptage des validations enrôlées, et refus d'une preuve manquante.
- Implémentation attendue: créer `docs/governance/m013_config_audit.md`, `uv run --locked gate`, enrôler les validations M13-config dans `uv run --locked gate` et `uv run --locked gate`, mettre à jour `docs/traceability/matrix.md` et compléter `docs/tasks/milestone_013-config/journal.md`.
...
- Dépendances: T-001 à T-007; ADR-016; `uv run --locked gate`; `uv run --locked gate`.
