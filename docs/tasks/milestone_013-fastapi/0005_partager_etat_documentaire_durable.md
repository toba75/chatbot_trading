...
- Tests d'acceptation à écrire: `uv run --locked gate`, couvrant enregistrement, redémarrage logique, relecture et job visible par un second processus.
- Tests unitaires à écrire: `uv run --locked gate`, couvrant idempotence, doublon binaire, transaction job/processing run, concurrence et original bit à bit.
