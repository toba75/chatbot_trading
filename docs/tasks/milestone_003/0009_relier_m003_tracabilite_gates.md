...
- Portée DDD: matrice `REQ-M003-*`, journal M-003, définition de terminé, ADR applicables, gates PowerShell, contrôles d'architecture, métriques d'ingestion et logs d'audit sans contenu documentaire.
- Scénario BDD:
...
- Tests d'acceptation à écrire: un test `tests/m003/validate_m003_traceability_acceptance.ps1` qui échoue tant qu'une exigence M-003 n'a pas de ligne dans `docs/traceability/matrix.md` avec test, commande, code, ADR ou justification et preuve d'audit.
