...
- Garde-fous: ne pas accepter une branche locale M-007 comme preuve de M-007 fusionné; ne pas contourner les validateurs de précondition historiques; ne pas déclarer GREEN si `uv run --locked gate` reste RED.

## Blocages Ou Préconditions
- État GREEN/RED connu: avant création de M-008, `uv run --locked gate` est RED sur `uv run --locked gate`, car `uv run --locked gate` refuse la branche `codex/plan-milestone-m008-conversation-produit`; `uv run --locked gate` est GREEN avec `16 validation(s), 0 test(s)`; `uv run --locked gate` est GREEN avec `8 milestone(s), 77 tâche(s) contrôlée(s)`; `git diff --check` est GREEN.
...
- Tests d'acceptation à écrire: `uv run --locked gate`, qui échoue tant que la branche M-008 n'est pas acceptée, que M-007 n'est pas visible dans `master` et que le rapport de précondition M-008 n'existe pas.
- Tests unitaires à écrire: tests de `uv run --locked gate` pour M-007 absent de `master`, branche ne contenant pas `master`, `origin/master` divergent, validateur amont rejetant M-008, gate RED, timeout non concluant et rapport GREEN sans sorties de commande.
- Implémentation attendue: créer `uv run --locked gate`, créer `docs/governance/m008_precondition_green.md`, corriger les listes ou règles de branches des validateurs de précondition amont sans changer leur sens, enrôler les tests M-008 et obtenir `uv run --locked gate` et `uv run --locked gate` GREEN.
...
- Dépendances: `master`; `origin/master`; `docs/tasks/milestone_007`; `docs/specs/m007_reponse_documentaire_verifiee.md`; `uv run --locked gate`; `uv run --locked gate`; `uv run --locked gate`.
- Commandes de validation: `git fetch origin --prune`; `git ls-tree -r --name-only master -- docs/tasks/milestone_007 docs/specs/m007_reponse_documentaire_verifiee.md scripts tests/m007`; `uv run --locked gate`; `uv run --locked gate`; `uv run --locked gate`.
