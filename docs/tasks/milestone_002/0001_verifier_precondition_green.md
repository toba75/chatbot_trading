...
- État GREEN/RED connu: au 2026-06-25, `uv run --locked gate` est GREEN avec 7 validations et 31 tests; `uv run --locked gate` est GREEN avec 7 validations.
...
- Tests d'acceptation à écrire: un test de précondition M-002 qui exécute `uv run --locked gate` et `uv run --locked gate`, vérifie la présence de `docs/tasks/milestone_001` dans `master` et échoue explicitement si une gate est RED.
...
- Dépendances: M-000; M-001; `uv run --locked gate`; `uv run --locked gate`; `uv run --locked gate`; `uv run --locked gate`.
- Commandes de validation: `uv run --locked gate`; `uv run --locked gate`; `git ls-tree -r --name-only master -- docs/tasks/milestone_000 docs/tasks/milestone_001`.
