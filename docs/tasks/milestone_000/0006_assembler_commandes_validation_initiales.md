...
- Invariants critiques: `uv run --locked gate` et `uv run --locked gate` existent; une commande échoue si un validateur requis manque; une commande ne masque pas un RED; la sortie nomme le validateur en échec.
...
- État GREEN/RED connu: au début de M-000, les commandes `uv run --locked gate` et `uv run --locked gate` sont absentes; `uv run --locked gate` est la seule validation existante.
- Présence des milestones amont dans master: M-000 n'a aucune dépendance amont.
- Décisions manquantes: ADR-010 documente la décision durable de gates uv run --locked gate
...
- Scénario BDD:
  ...
  - When `uv run --locked gate` et `uv run --locked gate` sont exécutés.
...
- Tests d'acceptation à écrire: un test qui exécute `uv run --locked gate` et `uv run --locked gate` en succès sur le dépôt valide, puis vérifie qu'une validation manquante ou échouée provoque un code de sortie non nul et un message ciblé.
...
- Implémentation attendue: créer `uv run --locked gate` et `uv run --locked gate`; brancher les validateurs produits par T-002 à T-005; documenter le périmètre exact des commandes M-000.
