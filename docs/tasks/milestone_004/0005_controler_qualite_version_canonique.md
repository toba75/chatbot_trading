...
- Tests d'acceptation à écrire: un test `uv run --locked gate` couvrant échantillonnage critique pré-conversion, comparaison de routes, statut `RETRY_WITH_ALTERNATIVE_ROUTE`, page omise, incohérence numérique, signe altéré, tableau incomplet et source quarantinée non publiable.
