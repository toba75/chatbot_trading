...
- Tests d'acceptation à écrire: `uv run --locked gate`, qui échoue tant qu'un résultat négatif peut être supprimé, qu'un échec peut disparaître, qu'une invalidation peut manquer de justification, qu'une archive logique rend le résultat inaccessible, ou qu'une correction peut réutiliser le même `ExperimentId` sans créer une nouvelle expérience liée à l'expérience invalidée.
