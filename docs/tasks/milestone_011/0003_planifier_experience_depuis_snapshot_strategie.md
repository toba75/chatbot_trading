...
- Tests d'acceptation à écrire: `uv run --locked gate`, qui échoue tant qu'une expérience peut être planifiée depuis une référence mutable, un snapshot absent, un snapshot non compilable ou un payload SD interne, ou tant qu'une expérience `PLANNED` peut être supprimée ou écrasée dans le registre.
