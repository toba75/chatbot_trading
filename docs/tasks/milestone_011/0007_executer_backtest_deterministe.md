...
- Tests d'acceptation à écrire: `uv run --locked gate`, qui échoue tant que le moteur peut recevoir une référence mutable, que les métriques peuvent venir d'un LLM, qu'une sortie sans hash est acceptée, qu'une lecture hors `DataSnapshotId` est possible ou qu'un contrôle minimal requis n'a pas de statut explicite.
