...
- Garde-fous: ne pas ignorer `uv run --locked gate`, `uv run --locked gate`, `uv run --locked gate` ni les preuves M-004; ne pas accepter une branche locale comme preuve d'un milestone amont fusionné.

## Blocages Ou Préconditions
- État GREEN/RED connu: `uv run --locked gate` GREEN; `uv run --locked gate` GREEN; `uv run --locked gate` GREEN avec 13 validation(s) et 91 test(s) après relance longue.
...
- Tests d'acceptation à écrire: `uv run --locked gate`, qui échoue tant que le validateur M-005, le rapport de précondition et la présence M-004 dans `master` ne sont pas vérifiés.
...
- Implémentation attendue: créer `uv run --locked gate`, produire `docs/governance/m005_precondition_green.md`, enrôler la précondition dans `uv run --locked gate` si la convention du dépôt l'exige, puis maintenir `uv run --locked gate` et `uv run --locked gate` en GREEN.
...
- Dépendances: `master`; `origin/master`; `docs/tasks/milestone_004`; `docs/specs/m004_version_canonique_publiee.md`; `uv run --locked gate`; `uv run --locked gate`.
- Commandes de validation: `git fetch origin --prune`; `git ls-tree -r --name-only master -- docs/tasks/milestone_004 docs/specs/m004_version_canonique_publiee.md scripts tests/m004`; `uv run --locked gate`; `uv run --locked gate`; `uv run --locked gate`.
