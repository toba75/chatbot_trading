...
- Risques: livrer le comportement sans preuve de traçabilité; compter les refus comme succès; oublier d'enrôler M-010 dans `uv run --locked gate` et `uv run --locked gate`.
...
- Dépendances: T-010; `docs/traceability/matrix.md`; `uv run --locked gate`; `uv run --locked gate`; `uv run --locked gate`.
