...
- Invariants critiques: chaque exigence M-009 possède test, commande, ADR ou justification; les tests M-009 sont enrôlés dans `scripts/test.ps1`; `scripts/lint.ps1` enrôle la spécification M-009; les frontières RA/EG/CV restent contrôlées.
...
- Portée DDD: matrice `docs/traceability/matrix.md`, enrôlement `scripts/test.ps1`, enrôlement `scripts/lint.ps1`, journal de milestone, validateurs M-009, preuves de métriques, contrôles d'architecture RA/EG/CV et documentation de clôture.
- Scénario BDD:
...
- Dépendances: T-001 à T-010; `scripts/test.ps1`; `scripts/lint.ps1`; `scripts/validate_traceability.ps1`; `scripts/validate_architecture_boundaries.ps1`; `docs/traceability/matrix.md`.
