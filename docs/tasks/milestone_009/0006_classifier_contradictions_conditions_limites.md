...
- Tests d'acceptation à écrire: `uv run --locked gate`, qui échoue tant que les convergences compatibles et les contradictions M-009 ne conservent pas conditions, limites et raisons publiques.
...
- Dépendances: T-005; `app/evidence_governance/domain/claim_relation.py`; `app/research_answering/domain/contradiction_assessment.py`; `uv run --locked gate`.
