...
- Tests d'acceptation à écrire: `uv run --locked gate`, qui échoue si une décision n'a pas de benchmark source, si un refus n'est pas conservé, si une promotion communautaire manque de comparaison sur une tâche LLM obligatoire, si un test scientifique RED est absent du rapport, si une décision EG, RA, CV ou SD manque, ou si un seuil est publié sans version.
