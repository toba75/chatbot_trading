...
- `uv run --locked gate` ne peut plus être GREEN avec les seuls micro-prompts LLM;
...
- Implémentation attendue: faire appeler les validations T-002 à T-009 par `uv run --locked gate` ou créer `uv run --locked gate` puis l'enrôler dans `uv run --locked gate` et le rapport V1.
